...
const PeopleTable = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const peopleListWithFullNames = response.data.map((person: PersonInfo) => {
                  // Find personOne and personTwo objects based on personIdOne and personIdTwo
                  const firstParent = response.data.find((p: PersonInfo) => p.id === person.firstparent_id);
                  const secondtParent = response.data.find((p: PersonInfo) => p.id === person.secondparent_id);
                
                  // Construct the new object with added full names
                  return {
                    ...person,
                    firstParentFullName: firstParent ? `${firstParent.firstname} ${firstParent.lastname}` : null,
                    secondtParentFullName: secondtParent ? `${secondtParent.firstname} ${secondtParent.lastname}` : null
                  };
                });
    ...
    const deleteHandler = async (e: React.MouseEvent<HTMLButtonElement>, personId: number) => {
      e.stopPropagation();
      try{
        ...
        if(response.status === 200) {
          ...
          setPeopleList((prevList) => prevList.filter(
            (item: PersonInfoFullNames) => item.id !== personId
          ));
        }
      } catch (error) {
